...
Both formats can be mixed in the same `permissionMapping` array.
...
The YAML file format additionally supports action-only entries (`['read']`) for broad matching, since YAML-sourced conditions are not editable through the UI.
